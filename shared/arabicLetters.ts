 ...
 * nothing at all.
 *
 * This is data with no dependencies, so it lives in `shared/` where the Qaida
 * audio manifest can enumerate the recordings the curriculum needs without
 * reaching into the client bundle. `client/src/lib/arabicLetters.ts` re-exports
 * it, and holds the playback paths that do depend on the client.
 */
