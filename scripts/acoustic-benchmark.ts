...
} from "../server/acoustic.benchmark";
...
if (process.env.QURAN_EVALUATOR_URL && manifestPath) {
  ...
  ) as { recordings: ManifestEntry[] };
  for (const entry of [...manifest.recordings].sort((a, b) =>
    ...
    if (!fixture) continue;
    const audioBase64 = (await readFile(resolve(entry.recordingPath))).toString(
      "base64"
    );
    const review = await evaluateQuranAwareAudio({
      audioBase64,
      mimeType: entry.mimeType ?? "audio/webm",
      expectedArabic: fixture.expectedArabic,
      surah: fixture.surah,
      ayah: fixture.ayah,
    ...
    predictions[fixture.id] = {
      ...
      findingKind: review.findings[0]?.kind ?? null,
    };
  }
}
...
if (process.env.QURAN_EVALUATOR_URL) {
  const rate = 16_000;
  const parts = [
    { ms: 450, amplitude: 0.25 },
    { ms: 1_100, amplitude: 0 },
    { ms: 450, amplitude: 0.25 },
  ];
  const samples = parts.flatMap(part =>
    Array.from(
      { length: (part.ms * rate) / 1000 },
      (_, i) => part.amplitude * Math.sin((2 * Math.PI * 220 * i) / rate)
    )
  );
  ...
  data.copy(wav, 44);
  const pipeline = await evaluateQuranAwareAudio({
  ...
  console.log(
    `Pipeline/contract validation (synthetic tone/silence; not pronunciation accuracy): ${pipeline.status}, ${pipeline.findings.length} contract finding(s)`
  );
}
